...
def evaluator(x: Melody) -> float:
    return (0.8 * F.interval_score(x) + 0.4 * F.rhythm_score(x) +
            0.6 * F.tonality_score(x, 'harmonic') + 0.2 * F.stable_score(x) + 0.6 * F.boundary_score(x)
            - F.density_penalty(x) -F.stop_penalty(x) - F.rest_penalty(x) - F.consecutive_penalty(x, 7) -
            F.range_penalty(x, 18) - F.variety_penalty(x, 5) - 0.5 * F.lonely_penalty(x))
...
if __name__ == '__main__':
    ...
    print(melody)
    print("Scores interval:{:.2f}, tonality:{}, rhythm:{:.2f}, stable:{:.2f}, boundary:{:.2f}".format(
